...
#Read in Dataset
Dataset <- fread("./data/NgramWordTable.RData")
...
Word_Count1 <- function(TextInputA){
  ...
  if (nrow(AlternativeGuess) == 0) {
    ...
    colnames(AlternativeGuess) <<- c("Predicted Word", "Probability %")
    rownames(AlternativeGuess) <<- NULL
  }

  Guess_Output <- NgramsTable$w3[1]
  if (is.na(Guess_Output) | is.null(Guess_Output)) {
    Guess_Output <- "Sorry I could not predict the next word. You may have either entered a rare word or possibly mispelt it."
  }
